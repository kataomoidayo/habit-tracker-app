...
class SettingsActivity : AppCompatActivity() {
    ...
    class SettingsFragment : PreferenceFragmentCompat() {
        override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
            ...
            val mode = findPreference<ListPreference>(getString(R.string.pref_key_dark))
            mode?.setOnPreferenceChangeListener { preference, newValue ->
                val changeValue = DarkMode.valueOf(newValue.toString().uppercase(Locale.ROOT))
                updateTheme(changeValue.value)
                true
            }
        }
